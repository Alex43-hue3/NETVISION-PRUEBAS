...
(function(){
  ...
  async function getJSON(url, params={}, options={}){
    ...
    let lastError=null;
    const timeout=Number(options.timeout)||A.timeout;
    const retries=options.retries===undefined?A.retries:Math.max(0,Number(options.retries)||0);
    for(let attempt=0; attempt<=retries; attempt++){
      ...
      try{
        ...
        if(!r.ok){
          ...
          err.status=r.status;
          // Los errores 4xx/5xx de streamurl no deben provocar esperas largas.
          if(options.retryOnStatuses && !options.retryOnStatuses.includes(r.status)) throw err;
          throw err;
        }
  ...
  async function streamFromEmbed(embed, provider){
    ...
    try{
      // streamurl es el cuello de botella detectado por V20.5.
      // Una fuente que devuelve 500 no merece 3 reintentos de hasta 22 s.
      const data=await getJSON(join(provider.base,'streamurl'),{url:embed},{timeout:7000,retries:0});
  ...
  async function resolve(item,kind){
    ...
    let urls=[...d.urls];
    if(urls.length) return {...d,urls:[...new Set(urls)]};

    const candidates=[...new Set([...(d.embeds||[]),...(d.candidates||[])])]
      .filter(Boolean)
      .filter(c=>!urls.includes(c));

    // No procesamos candidatos uno detrás de otro. Lanzamos un pequeño grupo
    // en paralelo y terminamos en cuanto aparece una respuesta con URLs.
    // Así una fuente caída no bloquea las demás.
    const concurrency=3;
    let cursor=0;
    let found=[];
    let cancelled=false;
    ...
    async function worker(){
      while(!cancelled){
        ...
        const got=await streamFromEmbed(candidates[i],d.provider);
        if(got.length){
          found.push(...got);
          cancelled=true;
          return;
