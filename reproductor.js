...
// V20.4 — precarga real de detalles/fuentes durante la navegación.
// Mantiene una caché de sesión para evitar repetir resoluciones ya realizadas.
const NV_PRELOAD = new Map();
const NV_PRELOAD_TTL = 10 * 60 * 1000;
function preloadKey(item, mode){
  return `${mode}:${String(item?.id||item?.slug||item?.title||'').trim().toLowerCase()}:${item?.year||''}`;
}
function getPreloaded(key){
  const x=NV_PRELOAD.get(key);
  if(!x) return null;
  if(Date.now()-x.savedAt>NV_PRELOAD_TTL){NV_PRELOAD.delete(key);return null;}
  return x.promise;
}
function cachePreload(key,promise){
  NV_PRELOAD.set(key,{savedAt:Date.now(),promise});
  promise.catch(()=>{ const x=NV_PRELOAD.get(key); if(x?.promise===promise) NV_PRELOAD.delete(key); });
  return promise;
}
NV.prepareMedia=async(item,mode)=>{
  if(!item) return null;
  const key=preloadKey(item,mode);
  ...
  return cachePreload(key,promise);
};
NV.prepareEpisode=async(ep)=>{
  if(!ep) return null;
  const key=`episode:${String(ep.slug||ep.id||'').trim().toLowerCase()}:${ep.season||''}:${ep.number||''}`;
  const cached=getPreloaded(key);
  if(cached) return cached;
  const promise=(async()=>await NVApi.resolveEpisode(ep))();
  return cachePreload(key,promise);
};
...
function renderEpisodes(episodes){
 const panel=NV.$('#episodesPanel');
 if(!episodes.length){
  panel.classList.remove('hidden');
  NV.$('#episodeList').innerHTML='<div class="empty-catalog" style="padding:30px 10px;border:0"><i class="fa-solid fa-list"></i><p>No se encontraron episodios.</p></div>';
  return;
 }
 panel.classList.remove('hidden');
 const groups={};
 episodes.forEach(e=>(groups[e.season||'1']??=[]).push(e));
 ...
 NV.$('#episodeList').innerHTML=seasons.map(s=>`<div class="season-block"><h3>Temporada ${NV.esc(s)}</h3>${groups[s].map((e,i)=>`<button class="episode-item" data-season="${NV.esc(s)}" data-idx="${i}"><span>${NV.esc(e.number)}</span><strong>${NV.esc(e.title)}</strong><i class="fa-solid fa-play"></i></button>`).join('')}</div>`).join('');
 NV.$$('#episodeList .episode-item').forEach(b=>{
  const warm=()=>{const e=groups[b.dataset.season][+b.dataset.idx];NV.prepareEpisode?.(e).catch(()=>{});};
  b.onmouseenter=warm;
  b.onfocus=warm;
  b.onclick=async()=>{
   NV.$$('#episodeList .episode-item').forEach(x=>x.classList.remove('active'));
   b.classList.add('active');
   const e=groups[b.dataset.season][+b.dataset.idx];
   setLoading('Iniciando episodio…');
   try{
    const ex=await NV.prepareEpisode(e);
    const ok=await playSources(ex?.urls||[],ex?.embeds||[]);
    if(!ok){NV.show('#mediaEmpty');NV.$('#mediaEmpty strong').textContent='No se encontró una fuente reproducible para este episodio'}
   }catch(err){
    console.error(err);NV.show('#mediaEmpty');NV.$('#mediaEmpty strong').textContent='No se pudo reproducir el episodio'
   }finally{NV.hide('#mediaLoading')}
  };
 });
}
NV.openMedia=async(item,mode)=>{NV.stopMedia();NV.state.currentMedia={item,mode};NV.setView('media');setMedia(item,mode);resetMedia();NV.updateListButtons?.();if(mode==='series'){setLoading('Preparando serie…');try{const ex=await NV.prepareMedia(item,mode);renderEpisodes(ex?.episodes||[]);NV.hide('#mediaLoading');if(!(ex.episodes||[]).length){const ok=await playSources(ex.urls||[],ex.embeds||[]);if(!ok)NV.show('#mediaEmpty')}}catch(e){console.error(e);NV.hide('#mediaLoading');NV.show('#mediaEmpty');NV.$('#mediaEmpty strong').textContent='No se pudo cargar la serie'}return}setLoading('Iniciando reproducción…');try{const ex=await NV.prepareMedia(item,mode);const ok=await playSources(ex?.urls||[],ex?.embeds||[]);if(!ok){NV.show('#mediaEmpty');NV.$('#mediaEmpty strong').textContent='No se encontró una fuente reproducible'}}catch(e){console.error(e);NV.show('#mediaEmpty');NV.$('#mediaEmpty strong').textContent='No se pudo consultar la fuente';NV.$('#mediaEmpty small').textContent=e.message||'Error de conexión'}finally{NV.hide('#mediaLoading')}};
